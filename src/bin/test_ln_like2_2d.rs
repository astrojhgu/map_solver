...
fn main(){
    ...
    let lp1=logprob_ana(answer.as_slice().unwrap(), &psd_param, total_tod.as_slice().unwrap(), &ptr_mat, n_t, n_ch);
    ...
    for i in 0..100{
        ...
        let lp2=logprob_ana(answer2.as_slice().unwrap(), &psd_param2, total_tod.as_slice().unwrap(), &ptr_mat, n_t, n_ch);
    
        let diff=lp2-lp1;
        let (gx, gp)=logprob_ana_grad(answer.as_slice().unwrap(), &psd_param, total_tod.as_slice().unwrap(), &ptr_mat, n_t, n_ch);
        println!("gx {} gp {}", gx.len(), gp.len());
        let diff2=ArrayView1::from(&gx).dot(&dx)+ArrayView1::from(&gp).dot(&dp);
        println!("{} {} {} {} {}",lp2, lp1, diff, diff2, 2.0*(diff2-diff).abs()/(diff.abs()+diff2.abs()));
    }
}
