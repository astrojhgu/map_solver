...
impl MappingProblem {
    pub fn new(ptr_mat: CsMat<f64>, corr_noise_cov: Array1<f64>, white_noise_cov:Option<Array1<f64>>, tod: Array1<f64>) -> MappingProblem {
    ...
    pub fn solve_sky(&self, mut cb: Option<&mut dyn FnMut(&Array1<f64>)>) -> Array1<f64> {
        ...
        let b = sp_mul_a1(
            ...
            Array1::from(
                if let Some(ref w)=self.white_noise_cov{
                    deconv(self.tod.as_slice().unwrap(), fnoise.as_slice()).into_iter().zip(w.iter()).map(|(x, &y)|{
                        x/y
                    }).collect::<Vec<_>>()
                }else{
