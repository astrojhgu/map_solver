...
fn batch_deconv(data: &[f64], k: &[Array1<num_complex::Complex<f64>>]) -> Array1<f64> {
    ...
    for k1 in k.iter() {
        result
            .slice_mut(s![cnt..cnt + (k1.len() - 1) * 2])
            .assign(&Array1::from(deconv(
                &data[cnt..cnt + (k1.len() - 1) * 2],
                k1.as_slice().unwrap(),
            )));
        cnt += (k1.len() - 1) * 2;
    }
...
impl MappingProblem {
    pub fn new(
        ...
        assert!(
            ...
            "num of pixels not equal"
        );

        assert!(ptr_mat
            .iter()
            .zip(noise_cov.iter())
            .all(|(m, n)| { m.rows() == n.len() }));
    ...
    pub fn solve_sky(
        ...
        let A: Box<dyn Fn(ArrayView1<f64>) -> Array1<f64>> =
            Box::new(|x: ArrayView1<f64>| -> Array1<f64> {
                self.apply_ptr_mat_t(
                    batch_deconv(self.apply_ptr_mat(x).as_slice().unwrap(), &fnoise).view(),
                )
            });
        ...
        let b = self.apply_ptr_mat_t(batch_deconv(&concated_tod, &fnoise).view());
