    ...
    pub fn get_logprob<'a>(
        ...
        move |p: &LsVec<f64, Vec<f64>>| {
            ...
            self.ptr_mat
                ...
                .map(|(p, t)| logprob_ana(&sky, &psp, t, p))
                .sum::<f64>()
            /*
    ...
    pub fn get_logprob_grad<'a>(
        ...
        move |p: &LsVec<f64, Vec<f64>>| {
            ...
            let grads: Vec<_> = self
                ...
                .par_iter()
                .zip(self.tod.par_iter())
                .map(|(p, t)| logprob_ana_grad(&sky, &psp, t, p))
                .collect();
    ...
    pub fn get_logprob_grad_psp<'a>(
        ...
        move |psp: &LsVec<f64, Vec<f64>>| {
            ...
            let grads: Vec<_> = self
                ...
                .map(|(p, t)| logprob_ana_grad(&sky, psp, t, p))
                .collect();
            let (_gx, gp) = grads.into_iter().fold(
                (
                    LsVec(vec![0.0_f64; sky.len()]),
                    LsVec(vec![0.0_f64; psp.len()]),
                ),
