...
use ndarray::{s, Array1, Array2, ArrayView1, ArrayView2, ArrayViewMut2, azip};
        ...
        for n in 0..N{
            for k in 0..M{
                for l in 0..N{
                    let p=n*M+m;
                    let q=l*M+k;
                    let f=T::from_usize(k*m).unwrap()/T::from_usize(M).unwrap()+T::from_usize(l*n).unwrap()/T::from_usize(N).unwrap();
...
pub fn ps_model<T>(ft: &[T], fch: &[T], a_t:T, ft_0: T, alpha_t:T , fch_0: T, alpha_ch: T, b: T, w: T, e:T)->Array2<T>
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>
{
    let a_ch=T::one();
...
pub fn dps_model_df0_t<T>(ft: &[T], fch: &[T], a_t:T, ft_0: T, alpha_t:T, fch_0: T, alpha_ch: T, b: T, w: T, e:T)->Array2<T>
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>
{
    let a_ch=T::one();
...
pub fn dps_model_df0_ch<T>(ft: &[T], fch: &[T], a_t:T, ft_0: T, alpha_t:T , fch_0: T, alpha_ch: T, b: T, w: T, e:T)->Array2<T>
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>
{
    let a_ch=T::one();
    let x=ft.iter().map(|&f| pl(f, a_t, ft_0, alpha_t, w, e)).collect::<Vec<_>>();
...
pub fn dps_model_dalpha_t<T>(ft: &[T], fch: &[T], a_t:T, ft_0: T, alpha_t:T , fch_0: T, alpha_ch: T, b: T, w: T, e:T)->Array2<T>
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>
{
    let a_ch=T::one();
...
pub fn dps_model_dalpha_ch<T>(ft: &[T], fch: &[T], a_t:T, ft_0: T, alpha_t:T ,  fch_0: T, alpha_ch: T, b: T, w: T, e:T)->Array2<T>
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>
{
    let a_ch=T::one();
    let x=ft.iter().map(|&f| pl(f, a_t, ft_0, alpha_t, w, e)).collect::<Vec<_>>();
...
pub fn dps_model_db<T>(ft: &[T], fch: &[T], a_t:T, ft_0: T, alpha_t:T , fch_0: T, alpha_ch: T, b: T, w: T, e:T)->Array2<T>
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>
{
    let a_ch=T::one();
    let two=T::one()+T::one();
...
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>
{
    ...
    X.iter().zip(psd.iter()).map(|(y,&p)| {
        let r=y.norm_sqr()/p/T::from_usize(n_t*n_ch).unwrap();
        r
    }).fold(T::zero(), |x, y|{x+y})
}
...
pub fn ln_likelihood(x: &[f64], y: &[f64], psd: ArrayView2<f64>, ptr_mat: &CsMat<f64>, n_t: usize, n_ch: usize)->f64
{
    let nout=y.len();
...
pub fn logprob_ana_grad(x: &[f64], psp: &[f64], tod: &[f64], ptr_mat: &CsMat<f64>, n_t: usize, n_ch: usize)->(Vec<f64>, Vec<f64>){
    ...
    let dpsd_dalpha_t=dps_model_dalpha_t(&ft, &fch, a_t, ft_0, alpha_t, fch_0, alpha_ch, b, PS_W, PS_E);
    let dpsd_dalpha_ch=dps_model_dalpha_ch(&ft, &fch, a_t, ft_0, alpha_t, fch_0, alpha_ch, b, PS_W, PS_E);
...
pub fn ln_likelihood_grad(x: &[f64], y: &[f64], psd: ArrayView2<f64>, ptr_mat: &CsMat<f64>, n_t: usize, n_ch: usize)->(Vec<f64>, Vec<f64>){
    let noise=&ArrayView1::from(y)-&sp_mul_a1(&ptr_mat, ArrayView1::from(x));
...
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32> + std::default::Default
{
    let n=x.len();
...
where 
    ...
    for i in 0..n{
        for j in 0..n{
            ...
            //result[(i,j)]=x[k];
            result.slice_mut(s![i*nr1..(i+1)*nr1, j*nc1..(j+1)*nc1]).assign(&x[k]);
        }
    }
    result
}
...
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>+std::default::Default
{
    ...
    let xb=cov.chunks(n_t).map(|x1|{circulant_matrix(x1)}).collect::<Vec<_>>();
...
mod tests {
    ...
    fn get_psd()->ndarray::Array2<f64>{
        ...
        let ft_min=0.1;
        let fch_min=1.0/n_ch as f64;
        let ft:Vec<_>=(0..(n_t+1)/2).chain(-n_t/2..0).map(|i| i as f64 * ft_min).collect();
        let fch:Vec<_>=(0..(n_ch as isize+1)/2).chain(-(n_ch as isize)/2..0).map(|i| i as f64 * fch_min).collect();
        let (a_t, ft_0, alpha_t)=(3.0, ft_min*2 as f64, -1.0);
        let (fch_0, alpha_ch)=(ft_min*2 as f64, -1.0);
        let b=0.1;
        super::ps_model(&ft, &fch, a_t, ft_0, alpha_t, fch_0, alpha_ch, b, 1e-9, 1e-9)
    }
    ...
    fn test_dxsx_dx(){
        ...
        let dx=1e-8;
        x1[m]+=dx;
        let direct_g=(super::ln_xsx(x1.view(), psd.view())-super::ln_xsx(x.view(), psd.view()))/dx;
    ...
    fn test_dxsx_dp(){
        ...
        //psd1[3]+=dp;
        //println!("{:?}", psd1);
        let direct_g=(super::ln_xsx(x.view(), psd1.view())-super::ln_xsx(x.view(), psd.view()))/dp;
    ...
    fn test_mvn_dlp_dp(){
        ...
        println!("a2: {} {}", (lp2a-lp1a)/delta, super::dhalf_ln_xsx_dp(x.view(), psd.view())[m]);
        
        println!("b2: {} {}", (lp2b-lp1b)/delta, super::dhalf_lndet_dps(psd.view())[m]);
    ...
    fn test_mvn_dlp_dx(){
        ...
        println!("A1: {} {}", (lp2a-lp1a)/delta, super::dhalf_ln_xsx_dx(x.view(), psd.view())[m]);
