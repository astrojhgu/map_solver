...
{
    let n = x.len();
    let mut y: Vec<_> = x.iter().map(Complex::<T>::from).collect();
...
pub fn dps_model_da<T>(f: T, a: T, _b: T, f0: T, alpha: T, w: T, e: T) -> T
where
    T: Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>,
{
    let two = T::one() + T::one();
...
pub fn dps_model_db<T>(_f: T, _a: T, b: T, _f0: T, _alpha: T, _w: T, _e: T) -> T
where
    T: Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>,
{
    let two = T::one() + T::one();
...
pub fn dps_model_df0<T>(f: T, a: T, _b: T, f0: T, alpha: T, w: T, e: T) -> T
where
    T: Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>,
{
    let two = T::one() + T::one();
...
pub fn dps_model_dalpha<T>(f: T, a: T, _b: T, f0: T, alpha: T, w: T, e: T) -> T
where
    T: Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>,
{
    let two = T::one() + T::one();
    ...
    T: Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>,
{
    ...
    X.iter()
        .zip(psd.iter())
        .map(|(y, &p)| y.norm_sqr() / p / T::from_usize(n).unwrap())
        .fold(T::zero(), |x, y| x + y)
}
    ...
    T: Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>,
{
    ...
    fft(&mut x_c, &mut X);
    let mut px: Vec<_> = psd.iter().zip(X.iter()).map(|(&p, &x)| x / p).collect();
    ...
    T: Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>,
{
    ...
    X.iter()
        .zip(psd.iter())
        .map(|(&x, &p)| -x.norm_sqr() / two / T::from_usize(n).unwrap() / p.powi(2))
        .collect()
}
    ...
    T: Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>,
{
    ...
    let dydp: Vec<_> = dhalf_ln_xsx_dp(x, psd)
        .into_iter()
        .zip(dhalf_lndet_dps(psd).into_iter())
        .map(|(dy1, dy2)| -dy1 - dy2)
        .collect();
    (dydx, dydp)
}
...
pub fn smoothness(psd: &[f64], k: f64) -> f64 {
    psd.windows(3)
        .map(|p| -(p[0].ln() + p[2].ln() - 2.0 * p[1].ln()).powi(2) / (2.0 * k.powi(2)))
        .sum::<f64>()
}
    ...
    for q in 0..y.len() {
        for i in 1..y.len() - 1 {
            result[q] = -(y[i + 1].ln() + y[i - 1].ln() - 2.0 * y[i].ln())
                * (delta(i + 1, q) / y[i + 1] + delta(i - 1, q) / y[i - 1]
                    - 2.0 * delta(i, q) / y[i])
                / k.powi(2);
        }
    }
...
pub fn logprob_ana(x: &[f64], psp: &[f64], tod: &[f64], ptr_mat: &CsMat<f64>) -> f64 {
    ...
    let psd = (0..(ntod + 1) / 2)
        .chain(-ntod / 2..0)
        .map(|i| ps_model(i as f64 * fmin, a, b, f0, alpha, PS_W, PS_E) + 1e-9)
        .collect::<Vec<_>>();
...
pub fn logprob_ana_grad(
    ...
    let dpsd_da = LsVec(
        (0..(ntod + 1) / 2)
            .chain(-ntod / 2..0)
            .map(|i| dps_model_da(i as f64 * fmin, a, b, f0, alpha, PS_W, PS_E))
            .collect::<Vec<_>>(),
    );
    let dpsd_db = LsVec(
        (0..(ntod + 1) / 2)
            .chain(-ntod / 2..0)
            .map(|i| dps_model_db(i as f64 * fmin, a, b, f0, alpha, PS_W, PS_E))
            .collect::<Vec<_>>(),
    );
    let dpsd_df0 = LsVec(
        (0..(ntod + 1) / 2)
            .chain(-ntod / 2..0)
            .map(|i| dps_model_df0(i as f64 * fmin, a, b, f0, alpha, PS_W, PS_E))
            .collect::<Vec<_>>(),
    );
    let dpsd_dalpha = LsVec(
        (0..(ntod + 1) / 2)
            .chain(-ntod / 2..0)
            .map(|i| dps_model_dalpha(i as f64 * fmin, a, b, f0, alpha, PS_W, PS_E))
            .collect::<Vec<_>>(),
    );
...
mod tests {
    ...
    fn test_dxsx_dx() {
        ...
        let dx = 0.00000001;
        x1[m] += dx;
        let direct_g = (super::ln_xsx(x1.as_slice().unwrap(), &psd)
            - super::ln_xsx(x.as_slice().unwrap(), &psd))
            / dx;
    ...
    fn test_dxsx_dp() {
        ...
        //psd1[3]+=dp;
        //println!("{:?}", psd1);
        let direct_g = (super::ln_xsx(x.as_slice().unwrap(), &psd1)
            - super::ln_xsx(x.as_slice().unwrap(), &psd))
            / dp;
    ...
    fn test_mvn_grad() {
        ...
        let noise: ndarray::Array1<f64> =
            ndarray::Array1::<f64>::zeros(1024).map(|_| rng.sample(StandardNormal));
        ...
        for m in 0..100 {
            ...
            let diff = ndarray::ArrayView1::from(&gx).dot(&dx)
                + ndarray::ArrayView1::from(&gp).dot(&ndarray::ArrayView1::from(&dp));
