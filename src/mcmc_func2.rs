...
pub fn smooth_step_step<T>(x:T, w:T)->T
...
pub fn smooth_step_step_prime<T>(x:T, w: T)->T
where T: Float + FloatConst{
...
{
    let two=T::one()+T::one();
    let y=(f.powi(2)/(f0.powi(2)+e)).powf(alpha/two);
    let a2=a.powi(2);
    let b2=b.powi(2);
    a2+b2+a2*(y-T::one())*smooth_step_step(f-f0, w)
}
...
{
    let two=T::one()+T::one();
    let y=(f.powi(2)/(e + f0.powi(2))).powf(alpha/two);
    two*a*(T::one()+(y-T::one())*smooth_step_step(f-f0, w))
}
...
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>{
    let two=T::one()+T::one();
    let y=f.powi(2)/(e + f0.powi(2));

    -a.powi(2)*(((alpha*f0*y.powf((two + alpha)/two) * smooth_step_step(f - f0, w))/f.powi(2)) + (y.powf(alpha/two)-T::one()) * smooth_step_step_prime(f - f0,w))
}
...
where T:Float + FloatConst + NumAssign + std::fmt::Debug + FFTnum + From<u32>{
    ...
    a.powi(2)* y.powf(alpha/two)* y.ln() * smooth_step_step(f-f0, w)/two
}
